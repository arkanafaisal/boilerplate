        ...
        {isForgotPasswordSuccess ? (
          ...
          <>
            <h2 className="text-xl font-bold mb-4">
              {type === 'login' ? t('auth.loginTitle') : (type === 'signup' ? t('auth.signupTitle') : t('auth.forgotPasswordTitle'))}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                ...
                <input 
                  ...
                  disabled={isLoading} 
                  maxLength={type === 'signup' ? VALIDATION.USER.MAX_USERNAME : VALIDATION.USER.MAX_EMAIL} 
              ...
              <button type="submit" disabled={isLoading} className="mt-2 bg-[var(--foreground)] text-[var(--background)] py-2.5 font-bold text-sm hover:opacity-80 transition-opacity">
                {isLoading ? 'Processing...' : (type === 'login' ? t('auth.btnLogin') : (type === 'signup' ? t('auth.btnSignup') : t('auth.btnForgotPassword')))}
              </button>
            </form>

            <div className="mt-5 flex flex-col gap-2 text-sm">
              <button onClick={() => setType(type === 'forgot-password' ? 'login' : (type === 'login' ? 'signup' : 'login'))} className="text-left hover:underline decoration-[var(--foreground)] underline-offset-4">
